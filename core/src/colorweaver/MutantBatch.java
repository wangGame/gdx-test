...
public class MutantBatch implements Batch {
    ...
    /** Constructs a new MutantBatch with a size of 1000, one buffer, and the default shader.
     * @see MutantBatch#MutantBatch(int, ShaderProgram) */
    public MutantBatch() {
        this(1000, null);
    }

    /** Constructs a MutantBatch with one buffer and the default shader.
     * @see MutantBatch#MutantBatch(int, ShaderProgram) */
    public MutantBatch(int size) {
        this(size, null);
    }

    /** Constructs a new MutantBatch. Sets the projection matrix to an orthographic projection with y-axis point upwards, x-axis
    ...
    @Override
    public void begin () {
        if (drawing) throw new IllegalStateException("MutantBatch.end must be called before begin.");
        renderCalls = 0;
    ...
    @Override
    public void end () {
        if (!drawing) throw new IllegalStateException("MutantBatch.begin must be called before end.");
        if (idx > 0) flush();
        lastTexture = null;
        drawing = false;
    ...
    public void draw (Texture texture, float x, float y, float originX, float originY, float width, float height, float scaleX,
        ...
        if (!drawing) throw new IllegalStateException("MutantBatch.begin must be called before draw.");
